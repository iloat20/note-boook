...
当前项目**不使用任何图表库（ECharts）**。年度报告与统计页都走 CSS/WXSS 渲染：

| 特性 | 年度报告 / 统计页 (CSS) | 备注 |
|------|------------------------|------|
| 渲染方式 | CSS flexbox / WXSS | 无 Canvas 图表上下文 |
| 动画 | CSS transitions | GPU 加速 |
| 内存占用 | 极低 | 无 gradientCache |
| 交互性 | 基础 | 满足展示需求 |
| 适用场景 | 简单数据展示（月度盈亏、组合分布） | K 线等复杂图当前未实现 |
| 维护成本 | 低 | — |

**设计建议**:
- 简单图表（年度、月度）→ CSS 方案（当前唯一方案）
- 复杂图表（K 线、组合图）如需实现，再评估引入 ECharts（**当前未接入任何图表库**）
