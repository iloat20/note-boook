...
- **持仓看板** — 自选股票 → 录入交易 → 自动计算持仓池（`quantity > 0`），按市场筛选（全部/A股/港股/美股）；实时行情（腾讯财经 API），滑动操作快速编辑/卖出/删除
...
持仓池 = Stock（自选股票）+ Transaction（交易记录）+ Dividend（分红）+ PriceCache（行情价格）实时计算得出：

1. 用户在录入交易时搜索/自选股票 → 存入 `stock_trade_stocks`
2. 系统根据买入/卖出交易计算 `quantity = 总买入 - 总卖出`
3. 首页筛选 `quantity > 0` 的股票展示为持仓池
4. 顶部 `liquid-slider` 按市场（全部/A股/港股/美股）筛选
