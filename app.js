...
App({
  ...
  _checkStorageQuota() {
    try {
      ...
      const limitMB = info.limitSize ? (info.limitSize / 1024).toFixed(1) : '未知'
      const pct = info.limitSize > 0 ? ((info.currentSize / info.limitSize) * 100).toFixed(1) : null

      if (pct !== null && parseFloat(pct) > 90) {
        console.warn('[App] 存储空间预警：已使用 ' + currentMB + 'MB / ' + limitMB + 'MB (' + pct + '%)')
        wx.showToast({
          title: '存储空间不足 (' + pct + '%)',
          icon: 'none',
          duration: 3000
