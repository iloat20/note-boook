...
function buildMarkdown() {
  ...
  if (transactions.length > 0) {
    ...
    transactions.forEach(function (t) {
      let stock = stockMap[t.stockId]
      let code = stock ? stock.code : '-'
      let name = stock ? stock.name : '-'
      let market = stock ? getMarketLabel(stock.market) : '-'
      let typeStr = t.type === 'BUY' ? '买入' : '卖出'
      let amount = (t.price || 0) * (t.quantity || 0)
      let dateStr = t.date ? fmtDate(new Date(t.date)) : '-'
      let strategies = (t.strategies && t.strategies.length) ? t.strategies.join(', ') : '-'
  ...
  if (dividends.length > 0) {
    ...
    dividends.forEach(function (d) {
      let stock = stockMap[d.stockId]
      let code = stock ? stock.code : '-'
      let name = stock ? stock.name : '-'
      let market = stock ? getMarketLabel(stock.market) : '-'
      let dateStr = d.date ? fmtDate(new Date(d.date)) : '-'
      let note = (d.note || '').replace(/\|/g, '\\|')
...
function exportMD() {
  ...
  try {
    ...
    let filePath = wx.env.USER_DATA_PATH + '/交易记录_' + timestamp + '.md'
