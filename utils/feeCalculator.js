...
const TRANSACTION_TYPE = constants.TRANSACTION_TYPE
const FEE_CONFIG = constants.FEE_CONFIG
...
function getFeeBreakdown(market, type, price, quantity) {
  ...
  switch (market) {
    ...
    case MARKETS.HK_SHARE: {
      ...
      const total = h.commission + h.stampDuty + h.transactionLevy + h.transactionFee + h.clearingFee
      return {
        total: parseFloat(total.toFixed(2)),
        items: [
          { name: "佣金", value: h.commission, rate: (config.commissionRate * 100).toFixed(3) + "%", min: config.commissionMin },
          { name: "印花税", value: h.stampDuty, rate: (config.stampDutyRate * 100).toFixed(2) + "%" },
          { name: "交易征费", value: h.transactionLevy, rate: (config.transactionLevyRate * 100).toFixed(4) + "%" },
          { name: "交易费", value: h.transactionFee, rate: (config.transactionFeeRate * 100).toFixed(3) + "%", min: config.transactionFeeMin },
          { name: "中央结算费", value: h.clearingFee, rate: (config.clearingFeeRate * 100).toFixed(3) + "%", min: config.clearingFeeMin }
        ]
      }
    }
    case MARKETS.US_SHARE: {
      ...
      return {
        ...
        items: [
          ...
          { name: "SEC费", value: u.secFee, rate: (config.secFeeRate * 100).toFixed(6) + "%", note: "仅卖出时收取，上限21.84" },
          { name: "TAF费", value: u.tafFee, note: "每股$" + config.tafFeePerShare + "，仅卖出时收取" }
        ]
      }
    }
    default:
